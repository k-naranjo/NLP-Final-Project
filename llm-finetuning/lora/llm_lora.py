...
class LLM_Lora(object):
    ...
    def train(self,
        ...
        # unless it's MetaICL
        self.prompter = Prompter(template_name=self.prompt_template_name, kshot=False)
        ...
        print(f"Loaded {len(train_data)} examples for train data.")
        
        # Create val set
        if len(data_path) > 0:
            print(f"Loading validation data from {data_path}")
            # val_data = load_dataset(data_path, split="validation") # Is this a unified split name?
            val_data = load_dataset(data_path, split="test") # Is this a unified split name?
            ...
            print(">>> Validation dataset is the test data.")
            print(f"Loaded {len(val_data)} examples for val data.")
        elif val_set_size > 0:
            # Split the training set if no val_file
            print(f"Getting some val examples from the training set")
            train_val = train_data.train_test_split(
                test_size=val_set_size, shuffle=True, seed=42
            )
            train_data = train_val["train"]
            val_data = train_val["test"]
            print(f"New train dataset size {len(train_data)}. Sampled validation dataset size {len(val_data)}")
        else:
    ...
    def predict(self,
                ...
                verbose: bool = False,
                text_output_only: bool = True,
                batch_size: int = 1,
                ):

        def handle_res(texts, items):
            # print(f"generated response: {text}")
            if label_set is not None:
                texts = [identify_label(text, label_set) for text in texts]
            ...
            for text, item in zip(texts, items):
                ...
                if text_output_only:
                    ...
                    output = {}
                    output['input'] = item['input']
                    output['completion'] = text
                    if 'output' in item:
                        output['reference'] = item['output']
        ...
        # Load the base model
        self.load_base_model()
        self.prompter = Prompter(template_name=self.prompt_template_name, kshot=kshot, verbose=True)
        ...
        outputs = []
        if batch_size > 1:
            self.tokenizer.padding_side = "right"  # If we don't set padding side to be right, a random token is generated "Љ"
            for item_batch in tqdm(list(collated(test_list, batch_size))):
                inputs = [item['input'] for item in item_batch]
                instructions = [item['instruction'] for item in item_batch]
                res = self._eval_batch(
                    instructions=instructions,
                    inputs=inputs,
                    max_new_tokens=max_new_tokens,
                )
                outs = handle_res(res, item_batch)
                outputs.extend(outs)
            return outputs
    ...
    def _eval_batch(self,
        ...
        # Format the prompt
        prompts = [ self.prompter.generate_prompt(
            instruction = inst,
            input = inp,
            demos = None,
            cutoff_length = self.cutoff_length,
        ) for inp, inst in zip (inputs, instructions)]
        ...
        with torch.no_grad():
            gen_output = self.model.generate(
                input_ids = input_ids,
                attention_mask = attention_mask, # Very very poor quality generations without adding attention mask
                generation_config = self.gen_config,
                return_dict_in_generate = True, # ?
                output_scores = True,
                max_new_tokens = max_new_tokens,
            )
        assert len(gen_output.sequences) == len(inputs['input_ids']), "Batched inference only implemented with beam size = 1"
        outputs = self.tokenizer.batch_decode(gen_output.sequences, skip_special_tokens=True) # Map to tokens
        return [
            self.prompter.get_response(output) for output in outputs  # Only keep the response part
        ]
    ...
    def generate_and_tokenize_prompt(self, data_point):
        ...
        if self.gen_prompt_count < PRINT_PROMPT_COUNT:
            print('-'*10)
            print(full_prompt)
            print('-'*10)
            self.gen_prompt_count += 1
